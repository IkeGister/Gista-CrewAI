...
class FirebaseAPITestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(service_account_path):
            ...
            try:
                ...
                print(f"Error initializing Firebase with environment variables: {e}")
                raise
        else:
    ...
    def test_link_operations(self):
        ...
        link_data = {
            ...
            "auto_create_gist": False  # Set to false to avoid creating a gist
        }
        ...
        self.assertIn(store_response.status_code, [200, 201])
        if store_response.status_code == 200:
            self.assertEqual(store_response.json()["message"], "Link stored successfully")
        else:
            self.assertIn("gistId", store_response.json())
    ...
    def test_gist_operations(self):
        ...
        self.assertIn(update_response.status_code, [200, 201])
        # Check that the message contains "Gist added successfully" (it might include additional info)
        self.assertIn("Gist added successfully", update_response.json()["message"])
